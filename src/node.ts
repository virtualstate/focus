...
export function toGenericNodeChildren(
  node: UnknownJSXNode | GenericNode
): TheAsyncThing<ChildNode[]> {
  return anAsyncThing(toGenericNodeChildrenInternal(toGenericNode(node)));
}

async function* toGenericNodeChildrenInternal(
  node: GenericNode
): AsyncIterable<ChildNode[]> {
  const { children } = node;
  ...
  async function* unionChildren(
    ...
    if (!fragments.length) {
      if (staticChildren.length) {
        yield staticChildren.map((value) =>
          isGenericChildNode(value) ? toGenericNode(value) : value
        );
      }
      return;
    }
    let nextChildren: (ChildNode[] | ChildNode)[] = [...children];

    for (const fragment of fragments) {
      nextChildren[+staticChildren.indexOf(fragment)] = undefined;
    }
    nextChildren = nextChildren.map((value) =>
      value
        ? isGenericChildNode(value)
          ? toGenericNode(value)
          : value
        : undefined
    );
    for await (const updates of union(
      fragments.map(async function* (
        fragment
      ): AsyncIterable<[number, ChildNode[]]> {
...
export function toGenericNodes(
  node: UnknownJSXNode
): TheAsyncThing<(GenericNode | ChildNode)[]> {
  return anAsyncThing(asNodes(toGenericNode(node)));

  async function* asNodes(
    node: GenericNode
  ): AsyncIterable<(GenericNode | ChildNode)[]> {
    if (!isFragment(node)) {
      return yield [node];
    } else {
...
export function toGenericChildNode<N = GenericNode>(
  node: unknown,
  options: ToGenericNodeOptions<N> = {}
): N | StaticChildNode {
...
export function toGenericNode<N = GenericNode>(
  node: unknown,
  options: ToGenericNodeOptions<N> = {}
): N {
