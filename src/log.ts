...
export async function logDescendantsSettledFromIterable(
  iterable: AsyncIterable<DescendantPromiseSettledResult[]>
) {
...
export async function logDescendantsSettledFromPromise(
  promise: Promise<DescendantPromiseSettledResult[]>
) {
  const states = await promise;
  logDescendantPromiseSettledResult(...states);
}

export function logDescendantPromiseSettledResult(
  ...states: DescendantPromiseSettledResult[]
) {
  function toNodeLogArray(node: unknown): unknown[] {
    if (isFragment(node)) {
      return ["fragment"];
    }
    return [name(node), properties(node)];
  }

  function toDescendantNodeArray(
    state: DescendantPromiseSettledResult
  ): unknown[] {
