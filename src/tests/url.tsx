...
const [url] = await children(
  <URL url="/example" base="https://example.com" search="?hmm=1" />
);
ok<GlobalURL>(url);
...
async function Input(
  { name, placeholder, ...rest }: InputOptions,
  input?: unknown
) {
  const items = await children(input);
  const urlItem = items.find(
    (item): item is GlobalURL => jsx.name(item) === "url"
  );
  const url = urlItem ?? (
    <URL url={`/inputs/${name}`} base="https://example.com" />
  );
  ...
  return (
    <input {...rest} name={name} placeholder={placeholder}>
      ...
      <Prompt
        ...
        env={`PROMPT_${change.constantCase(name)}`}
      />
    </input>
  );
}
