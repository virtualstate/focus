...
export async function Data({ data }: DataOptions = {}, input?: unknown) {
  ...
  function resolve(
    data: Uint8Array | unknown[] | string | number
  ): unknown[] | Uint8Array {
...
async function Write({ name }: WriteOptions, input?: unknown) {
  const resolved = await children(<Data>{input}</Data>);
...
const [success] = await descendants(<Write name={name}>{data}</Write>);
...
async function String(options?: unknown, input?: unknown) {
  const data = await children(<Data>{input}</Data>);
  ok(Array.isArray(data));
